...
2. **Policy bypass** — Safe mode blocks NSE (`--script`, `-A`, `-sC` forms), arbitrary file reads (`-iL`, `--iL`, …), most non-stdout outputs (`-oN file`, long `--oG` paths, …), resume/proxies/idle scan flags, and several long options that change data paths. See the detailed write-ups below for historical bypass classes.
